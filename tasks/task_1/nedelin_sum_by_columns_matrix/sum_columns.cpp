...
#include "task_1/nedelin_sum_by_columns_matrix/sum_columns.h"
...
std::vector<int> GetSumColsParallel(const std::vector<int>& matrix,
    ...
    MPI_Scatter(Matrix.data(), Rows * Cols, MPI_INT, Buffer.data(),
        Rows * Cols, MPI_INT, 0, MPI_COMM_WORLD);
    std::vector<int>Sum(Cols);
    std::vector<int> tmpSum = GetSumCols(Buffer, Rows, Cols);
    MPI_Reduce(tmpSum.data(), Sum.data(), Cols,
