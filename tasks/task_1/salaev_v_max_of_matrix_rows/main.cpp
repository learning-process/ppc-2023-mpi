...
TEST(Parallel_Max_Of_Matrix_Rows_MPI, Test_NegativeNumbers) {
    ...
    if (world.rank() == 0) {
        ASSERT_EQ(0, global_maxes[0]); // Максимум в первой строке на позиции 0 (-1)
        ASSERT_EQ(3, global_maxes[1]); // Максимум во второй строке на позиции 3 (-5)
        ASSERT_EQ(1, global_maxes[2]); // Максимум в третьей строке на позиции 1 (-9)
    }
}
