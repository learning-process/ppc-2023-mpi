...
#include <iostream>
#include <vector>
#include "task_2/nedelin_a_transmission_broadcast/broadcast.h"
...
void improved_broadcast(void* data, int count,
    ...
    if (rank == root) {
        if (tmp->left != nullptr) {
            MPI_Send(data, count, datatype, tmp->left->rank, 0, comm);
        }
        if (tmp->right != nullptr) {
            MPI_Send(data, count, datatype, tmp->right->rank, 0, comm);
        }
    } else {
