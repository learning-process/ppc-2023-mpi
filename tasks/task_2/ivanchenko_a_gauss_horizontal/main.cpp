...
TEST(GAUSS_HORIZONTAL, test_parallel_small) {
    ...
    std::vector<double>expected = gaussSequentional(A, b);
...
TEST(GAUSS_HORIZONTAL, test_parallel_small_11) {
    ...
    size_t size = 4;
    std::vector<double>A(size * size);
    A[0*size + 0] = 9.0; A[0*size + 1] = 1.0; A[0*size + 2] = 10.0; A[0*size + 3] = 6.0;
    A[1*size + 0] = 7.0; A[1*size + 1] = 1.0; A[1*size + 2] = 7.0; A[1*size + 3] = 7.0;
    A[2*size + 0] = 8.0; A[2*size + 1] = 8.0; A[2*size + 2] = 7.0; A[2*size + 3] = 9.0;
    A[3*size + 0] = 9.0; A[3*size + 1] = 7.0; A[3*size + 2] = 5.0; A[3*size + 3] = 6.0;
...
TEST(GAUSS_HORIZONTAL, test_parallel_random_fill_0) {
    ...
    std::vector<double>A = getRandomMatrix(4, 4);
    std::vector<double>b = getRandomVector(4);
...
TEST(GAUSS_HORIZONTAL, test_parallel_random_fill) {
    std::vector<double>A = getRandomMatrix(17, 17);
    std::vector<double>b = getRandomVector(17);
...
TEST(GAUSS_HORIZONTAL, test_parallel_random_fill_1) {
    std::vector<double>A = getRandomMatrix(19, 19);
    std::vector<double>b = getRandomVector(19);
    std::vector<double>expected = gaussSequentional(A, b);
    std::vector<double>actual = gaussParallel(A, b);
    for (int i = 0; i < expected.size(); i++) {
        EXPECT_FLOAT_EQ(expected[i], actual[i]);
...
TEST(GAUSS_HORIZONTAL, test_parallel_random_fill_2) {
    std::vector<double>A = getRandomMatrix(24, 24);
    std::vector<double>b = getRandomVector(24);
