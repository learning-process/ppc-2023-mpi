...
std::vector<int> getParallelOperations(const std::vector<int>& matrix1,
    ...
    if (size > row_count_matrix1 || size == 1) {
        return rank == 0 ?
            getSequentialOperations(matrix1, matrix2, row_count_matrix1,
                column_count_matrix1, row_count_matrix1) :
            std::vector<int>{};
    }
    ...
    if (rank == 0) {
        for (int proc = 1; proc < size; proc++) {
            ...
            std::vector<int> send_vector_b(local_count *
                row_count_matrix2, 0);
            for (int i = 0; i < send_vector_b.size(); i++) {
                int step_row = i % local_count;
                int step_column = (i / local_count) * column_count_matrix2;

                send_vector_b[i] = matrix2[step_matrix2 +
                    step_row + step_column];
            }

            MPI_Send(matrix1.data() + step_matrix1, local_count
                * column_count_matrix1,
    ...
    if (rank == 0) {
        ...
        MPI_Status status;
        MPI_Recv(local_vector_a.data(), static_cast<int>(local_size
            - remaining * column_count_matrix1),
            MPI_INT, 0, 1, MPI_COMM_WORLD, &status);
        MPI_Recv(local_vector_b.data(), static_cast<int>(local_size
            - remaining * column_count_matrix1),
            MPI_INT, 0, 2, MPI_COMM_WORLD, &status);
    }
    local_vector_b[local_size] = rank == 0 ? 0 :
        rank * local_count + remaining;
    ...
    for (int i = 0; i < size; i++) {
        int local_row_count_matrix1 = rank == 0 ? local_count
            + remaining : local_count;
        int local_column_count_matrix2 =
            (rank + i) % size == 0 ? local_count + remaining
            : local_count;
        ...
        for (int j = 0; j < tmp_res.size(); j++) {
            int step_row = local_vector_b[local_size] +
                j % local_column_count_matrix2;
            int step_column = (j / local_column_count_matrix2)
                * row_count_matrix1;
