...
TEST(Monte_Carlo_omp, Test_one_dimension) {
    ...
    double calc_value_integral_omp =
        getIntegralMonteCarloOmp(f, a, b, number_points);
    double error = std::abs(calc_value_integral - calc_value_integral_omp)/
...
TEST(Monte_Carlo_omp, Test_three_dimension) {
    const std::function<double(const std::vector<double>)> f = []
        (const std::vector<double>& x) -> double {
            return x[0] * x[1] * x[1] * x[2] + x[1] * x[2] / 2.0;
        };
